...
class RobotControlNode(Node):
    ...
    def controls(self):
        ...
        while True:
            ...
            object_center = object_queue.popleft()
            # Add to completed list
            COMPLETED.append(object_center['id'])
            action_format['value']['positionx'] = object_center['center'][0]
            ...
            object_id += 1
            attach['value']['object'] = f"box_orange_{object_id}"
            node.control(attach)
...
def spin_node():
    # rclpy.init()
    overheadcamnode = OverheadCamSub()
    try:
