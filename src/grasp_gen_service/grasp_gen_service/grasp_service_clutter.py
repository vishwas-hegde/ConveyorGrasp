...
class GenerativeGraspService(Node):
    ...
    def generate_grasp_callback(self, request, response):
        ...
        try:
            ...
            if grasp_type == "generate_grasp_grconvnet":
                  gs, depth_img_processed = self.grconvnet.process_data(self.rgb_image, self.depth_image, crop, self.latest_detections)  
                  # print("out of process")
                  # print("gs = ", gs, len(gs))
                  # print("id = ", type(idd))
                  flat_gs = []
                 
                  for grasp_list in gs: 
                        for grasp in grasp_list: 
                                    flat_gs.extend([
                                    grasp['center_x'], grasp['center_y'],
                                    grasp['width'], grasp['height'],
                                    grasp['angle'], int(grasp['id'])
                                    ])
                  ...
                  closest_grasp = self.plot_grasp(gs, self.latest_detections, idd)
    ...
    def plot_grasp(self, grasps, detections, idd):
        """
      ...
      #   print("Grasp List: ", grasp_list)
        for detection in detections.get("detections", []):
            ...
            for k, g in enumerate(grasp_list):
                  print(f"Grasp {k}: {g}")
                  centerx, centery, length, width, angle, grasp_id = g
                  #     print("Grasp ID: ", grasp_id, idd)
                  # if grasp_id == idd:
