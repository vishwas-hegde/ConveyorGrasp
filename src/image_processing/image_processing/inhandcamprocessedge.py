...
# from tf2_ros import quaternion_matrix
...
class InHandCamNode(Node):
    ...
    def get_object_center_and_dimensions(self):
        ...
        if rgb_image is not None:
            ...
            print(angle, self.rot_z)
            angle = angle + 90 + self.rot_z
    ...
    @staticmethod
    def transform_to_matrix(quaternion):
    ...
    def convert_to_world_coordinates(self):
        ...
        if self.center is not None and rgb_image is not None:
            ...
            T_c_h = np.array([H_c_h.transform.translation.x, H_c_h.transform.translation.y - 0.05, H_c_h.transform.translation.z])
            try:
